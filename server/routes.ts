...
// FIX: Changed import to use named export
import { uploadGeneral } from "./utils/upload"; 
...
export function setupRoutes(app: Express): Server {
  ...
  // NOTE: Modular routes available in server/routes/index.ts
 ...
 // Avatar upload -> Cloudinary
  ...
  // FIX: Updated to use uploadGeneral.single
  app.post("/api/upload", authenticate, uploadGeneral.single("file"), (req: any, res) => {
    if (!req.file) {
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      // Helper: fetch progress rows based on role/query, then merge story checkpoints
      let progressData: any[] = [];
      let checkpointFilter: any = {};

      if (role === "admin" && req.query.studentId) {
        const studentId = parseInt(req.query.studentId as string, 10);
        if (!Number.isFinite(studentId)) return res.status(400).json({ message: "Invalid studentId" });
        progressData = await db.query.progress.findMany({
          where: eq(schema.progress.userId, studentId),
          with: { book: true, user: { columns: { id: true, firstName: true, lastName: true, username: true } } },
          orderBy: desc(schema.progress.lastReadAt),
        });
        checkpointFilter = { userId: studentId };
      } else if (role === "admin" && !req.query.studentId) {
        progressData = await db.query.progress.findMany({
          with: { book: true, user: { columns: { id: true, firstName: true, lastName: true, username: true } } },
          orderBy: desc(schema.progress.lastReadAt),
        });
        ...
        progressData = await db.query.progress.findMany({
          with: { book: true, user: { columns: { id: true, firstName: true, lastName: true, username: true, approvalStatus: true, role: true } } },
          orderBy: desc(schema.progress.lastReadAt),
        });
        // teacher sees only approved students; restrict checkpoints similarly
        checkpointFilter = {};
      } else {
        // student
        progressData = await db.query.progress.findMany({
        ...
        checkpointFilter = { userId };
      }

      // Fetch story checkpoints matching the same visibility (include book & user)
      const checkpointWhere: any[] = [];
      if (checkpointFilter.userId) checkpointWhere.push(eq(schema.storyCheckpoints.userId, checkpointFilter.userId));
      // For teachers/admin with no user filter we fetch all checkpoints (will be filtered later)

      const checkpoints = await db.query.storyCheckpoints.findMany({
        where: checkpointWhere.length ? checkpointWhere[0] : undefined,
        with: { book: true, user: { columns: { id: true, firstName: true, lastName: true, username: true, approvalStatus: true, role: true } } },
        orderBy: desc(schema.storyCheckpoints.lastCheckpointAt),
      });

      // Build a map from progressData by bookId (latest row per book)
      const progressMap = new Map<number, any>();
      for (const p of progressData) {
        const bid = p.book?.id ?? p.bookId;
        if (bid) progressMap.set(bid, p);
      }

      // Merge checkpoints for books that don't have progress or are newer
      for (const cp of checkpoints) {
        const bid = cp.book?.id ?? cp.bookId;
        if (!bid) continue;
        const existing = progressMap.get(bid);
        const cpTime = cp.lastCheckpointAt ? new Date(cp.lastCheckpointAt).getTime() : 0;
        const existingTime = existing && existing.lastReadAt ? new Date(existing.lastReadAt).getTime() : 0;
        if (!existing || cpTime > existingTime) {
          // create a progress-like object from checkpoint
          const pseudo = {
            id: existing?.id ?? null,
            userId: cp.userId,
            bookId: bid,
            percentComplete: cp.percentComplete ?? 0,
            lastReadAt: cp.lastCheckpointAt ?? cp.createdAt ?? null,
            totalReadingTime: existing?.totalReadingTime ?? 0,
            book: cp.book,
            user: cp.user,
          };
          // For teacher views, only include checkpoints for approved students
          if (role === "teacher") {
            if (pseudo.user?.role === "student" && pseudo.user?.approvalStatus === "approved") progressMap.set(bid, pseudo);
          } else {
            progressMap.set(bid, pseudo);
          }
        }
      }

      const merged = Array.from(progressMap.values()).sort((a: any, b: any) => {
...
app.post(
  ...
  app.post("/api/reading-sessions/start", authenticate, async (req, res) => {
    try {
      let { bookId } = req.body || {};
      const userId = (req as any).user?.id;
      // Accept either numeric bookId or slug string
      if (!userId || (bookId == null && !req.body?.slug))
        return res.status(400).json({ success: false, message: "Missing userId or bookId/slug" });
      if (!bookId && req.body?.slug) {
        try {
          bookId = await resolveBookIdBySlug(String(req.body.slug));
        } catch (err: any) {
          return res.status(err?.status ?? 404).json({ success: false, message: err?.message || 'Book not found' });
        }
      }
