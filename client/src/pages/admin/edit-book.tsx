...
import { PageForm, PageFormValues, PageFormHandle } from '@/components/admin/PageForm';
...
export default function EditBook() {
  ...
  const pageFormRefs = useRef<Record<number, React.RefObject<PageFormHandle>>>({});
  ...
  const onSubmit = async (data: EditBookFormValues) => {
    try {
      ...
      const flushedPages: PageFormValues[] = [];
      for (const p of pages) {
        const ref = pageFormRefs.current[p.pageNumber];
        if (ref && ref.current && typeof ref.current.flush === 'function') {
          try {
            const latest = ref.current.flush();
            flushedPages.push(latest);
            continue;
          } catch (e) {
            // fall back
          }
      ...
      for (const page of flushedPages) await updatePagesMutation.mutateAsync(page);
  ...
  const handleAddPage = () => {
    ...
    for (let i = 0; i < existingPageNumbers.length; i++) {
      if (existingPageNumbers[i] !== newPageNumber) break;
      newPageNumber++;
    }
  ...
  if (isLoadingBook || isLoadingPages) {
    return (
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-slate-100 to-indigo-50/50 font-sans antialiased">
        <Header variant="admin" />
        <main className="flex-1 flex items-center justify-center">
          <div className="flex flex-col items-center gap-3 bg-white rounded-2xl p-6 border border-slate-200 shadow-xl">
            <Loader2 className="h-10 w-10 animate-spin text-blue-600" />
            <p className="text-base font-semibold text-slate-800">Loading book data...</p>
          </div>
        </main>
      </div>
    );
  ...
  if (!bookData && !isLoadingBook) {
    return (
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-slate-100 to-indigo-50/50 font-sans antialiased">
        <Header variant="admin" />
        <main className="flex-1 flex items-center justify-center">
          <div className="flex flex-col items-center gap-3 bg-white rounded-2xl p-6 border border-red-200 shadow-xl">
            <p className="text-base font-semibold text-red-600">❌ Book not found</p>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-slate-100 to-indigo-50/50 font-sans antialiased">
      ...
      >
        ...
        <div className="container mx-auto px-4 relative">
          <div className="flex items-center justify-center mb-3">
            <GraduationCap className="h-7 w-7 text-sky-400 mr-2" />
            <span className="text-sm font-semibold text-sky-400 tracking-wide uppercase">
              Adonai And Grace Inc.
            </span>
          </div>
          <h1 className="text-2xl md:text-3xl font-bold text-center">✏️ Edit Book</h1>
          <p className="text-slate-300 text-center mt-1">Update your educational content</p>

          <div className="mt-3 flex items-center justify-center text-[12px] text-slate-400 gap-2 font-medium">
      ...
      <main className="flex-1 py-6 container mx-auto px-4">
        ...
        {/* == Form Section == */}
        <motion.div variants={fadeIn} initial="hidden" animate="visible" className="border border-slate-200 bg-white rounded-2xl shadow-lg mb-16">
          <div className="border-b border-slate-200 p-5 rounded-t-2xl">
            <h3 className="text-lg font-bold text-slate-900 flex items-center">
              <Edit3 className="h-5 w-5 text-blue-600 mr-2" />
              Edit Book Details
            </h3>
            <p className="text-sm text-slate-500 mt-0.5">Update book details and content.</p>
          </div>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <motion.div variants={stagger} initial="hidden" animate="visible" className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  {/* LEFT: Fields + Description + Badges */}
                  <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4 auto-rows-min">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="!space-y-1">
                            <FormLabel className="font-semibold text-slate-800">📖 Title</FormLabel>
                            <FormControl>
                              <Input placeholder="Book title" className="border border-slate-200 focus:border-blue-500 shadow-sm" {...field} />
                            </FormControl>
                            <p className="text-[12px] text-slate-500 mt-1">Keep it short and descriptive.</p>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                    ...
                    <motion.div variants={itemFade}>
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="!space-y-1">
                            <FormLabel className="font-semibold text-slate-800">📚 Book Type</FormLabel>
                            <Select onValueChange={field.onChange} value={field.value}>
                              <FormControl>
                                <SelectTrigger className="border border-slate-200 focus:border-blue-500 shadow-sm">
                      ...
                      {form.watch('type') === 'educational' && (
                        ...
                        >
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem className="!space-y-1">
                                <FormLabel className="font-semibold text-slate-800">📋 Subject Category</FormLabel>
                                <Select onValueChange={field.onChange} value={field.value}>
                                  <FormControl>
                                    <SelectTrigger className="border border-slate-200 focus:border-blue-500 shadow-sm">
                                ...
                                <p className="text-[12px] text-slate-500 mt-1">Required for educational books.</p>
                                <FormMessage />
                              </FormItem>
                            )}
                          />
                    ...
                    <motion.div variants={itemFade}>
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="!space-y-1">
                            <FormLabel className="font-semibold text-slate-800">🎓 Grade Level</FormLabel>
                            <Select onValueChange={field.onChange} value={field.value}>
                              <FormControl>
                                <SelectTrigger className="border border-slate-200 focus:border-blue-500 shadow-sm">
                    ...
                    <motion.div variants={itemFade} className="md:col-span-2">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="!space-y-1">
                            <FormLabel className="font-semibold text-slate-800">🧠 Quiz Mode</FormLabel>
                            ...
                            <p className="text-[12px] text-slate-500 mt-1">Controls how students can attempt the quiz.</p>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                    ...
                    <motion.div variants={itemFade} className="md:col-span-2">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="font-semibold text-slate-800">🎵 Background Music URL (optional)</FormLabel>
                            <FormControl>
                              <Input
                            ...
                            <div className="mt-2">
                              <Input
                                type="file"
                                accept="audio/*"
                                className="border border-slate-200 focus:border-blue-500 text-slate-600 file:text-slate-700 file:font-semibold file:bg-slate-100 hover:file:bg-slate-200 file:border-0 file:mr-4 file:py-1 file:px-3 file:rounded-md cursor-pointer"
                                onChange={async (e) => {
                                ...
                                disabled={audioUploading}
                              />
                              {audioUploading && <p className="text-xs text-blue-600 mt-1">Uploading audio…</p>}
                            </div>
                            <FormMessage />
                          </FormItem>
                        )}
                    ...
                    <motion.div variants={itemFade} className="md:col-span-2">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="font-semibold text-slate-800">📝 Description</FormLabel>
                            <FormControl>
                              <Textarea
                                placeholder="Enter book description"
                                className="min-h-[96px] border border-slate-200 focus:border-blue-500 shadow-sm"
                                {...field}
                              />
                            </FormControl>
                            <p className="text-[12px] text-slate-500 mt-1">A brief overview students will see.</p>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                    ...
                    {/* BADGES */}
                    <motion.div variants={itemFade} className="md:col-span-2 border border-slate-200 bg-slate-50 rounded-xl p-5 mt-2">
                      <h3 className="text-base font-bold mb-3 text-slate-900">🏅 Badges & Award Rules</h3>

                      {isLoadingBadges || isLoadingBookBadges ? (
                        <div className="bg-white p-4 rounded-xl text-center border border-slate-200">
                          <Loader2 className="h-5 w-5 animate-spin inline-block mr-2 text-blue-600" />
                          <span className="text-slate-600 font-medium">Loading badges...</span>
                        </div>
                      ) : allBadges.length === 0 ? (
                        <div className="bg-white p-4 rounded-xl text-center border border-slate-200">
                          <p className="text-slate-500 font-medium">No badges available.</p>
                        </div>
                      ) : (
                                  ...
                                  <AnimatePresence initial={false} mode="popLayout">
                                    {m?.isEnabled && (
                                      <motion.div
                                        ...
                                        animate="visible"
                                        exit="exit"
                                        className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-slate-100 pt-3"
                                      >
                                        <div className="md:col-span-1">
                                          <label className="block text-sm font-semibold text-slate-800 mb-1.5">Award Method</label>
                                          <Select
                                            value={m.awardMethod}
                                        ...
                                        <div className="md:col-span-1">
                                          <label className="block text-sm font-semibold text-slate-800 mb-1.5">% Completion</label>
                                          <Input
                                            type="number"
                                            min={1}
                                            max={100}
                  ...
                  <motion.div variants={itemFade} className="lg:col-span-1">
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-5 h-full shadow-sm">
                      <div className="flex items-center gap-2 mb-4">
                        <ImageIcon className="h-5 w-5 text-blue-600" />
                        <h4 className="font-bold text-slate-900">Cover Image</h4>
                      </div>
                      ...
                      {form.watch('coverImage') ? (
                        ...
                        <div className="mb-4 aspect-[3/4] rounded-lg border-2 border-dashed border-slate-300 bg-white flex items-center justify-center text-sm text-slate-500 font-medium">
                          No cover selected
                        </div>
                      )}
                          ...
                          <FormItem>
                            ...
                            <div className="mt-3 flex items-center gap-2">
                              <Input
                                ...
                                className="border border-slate-200 focus:border-blue-500 text-slate-600 file:text-slate-700 file:font-semibold file:bg-slate-100 hover:file:bg-slate-200 file:border-0 file:mr-3 file:py-1 file:px-3 file:rounded-md cursor-pointer"
                                onChange={async (e) => {
                                  const file = e.target.files?.[0];
                                  if (!file) return;
                                  try {
                            ...
                            {coverUploading && <p className="text-xs text-blue-600 mt-1">Uploading cover…</p>}
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                ...
                {/* == Pages Section == */}
                <motion.div variants={itemFade} initial="hidden" animate="visible" className="border border-slate-200 bg-slate-50 rounded-xl p-5 shadow-sm">
                  ...
                  {pages.length === 0 ? (
                    <motion.div variants={itemFade} initial="hidden" animate="visible" className="bg-white p-8 rounded-xl text-center border border-slate-200 shadow-sm">
                      <p className="text-slate-500 font-medium mb-4">📝 No pages added yet</p>
                      <Button
                        type="button"
                        onClick={handleAddPage}
                        className="bg-slate-900 hover:bg-slate-800 text-white font-semibold transition-transform hover:-translate-y-0.5"
                      >
                    ...
                    <div className="space-y-4">
                      <AnimatePresence initial={false}>
                        {pages
                          ...
                          .map((page) => (
                            ...
                            >
                              {(() => {
                                if (!pageFormRefs.current[page.pageNumber]) pageFormRefs.current[page.pageNumber] = ({ current: null } as React.RefObject<PageFormHandle>);
                                return (
                                  <PageForm
                                    ref={pageFormRefs.current[page.pageNumber]}
                                    initialValues={page}
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={handleAddPage}
                        className="w-full py-6 border border-slate-300 text-slate-700 hover:bg-slate-100 font-semibold transition-all hover:border-slate-400"
                      >
                        <Plus className="h-4 w-4 mr-2 text-slate-500" /> ✨ Add Another Page
                      </Button>
                    </div>
                  )}
                </motion.div>
        ...
        >
          <div className="container max-w-5xl mx-auto">
            <div className="rounded-xl border border-slate-200 bg-white/95 backdrop-blur-sm p-4 shadow-xl flex items-center justify-end gap-3">
              <Button
                variant="outline"
                onClick={() => navigate('/admin/books')}
                className="border border-slate-300 text-slate-700 hover:bg-slate-50 font-medium"
              ...
              <Button
                ...
                className="min-w-[140px] bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold transition-transform hover:-translate-y-0.5 shadow-md"
              >
