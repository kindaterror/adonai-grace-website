...
export default function StudentDashboard() {
  ...
  // Small static fallback map for the three exclusive 2D stories
  ...
  const EXCLUSIVE_2D_SLUGS = new Set(Object.keys(storyCoverMap));

  function slugFromProgress(p: any) {
    const s = p?.book?.slug;
  ...
  function coverUrlForProgress(p: any) {
    const book = p?.book;
    if (!book) return undefined;
    if (book.coverImage) return book.coverImage;
    const slug = slugFromProgress(p);
    if (slug && storyCoverMap[slug]) return storyCoverMap[slug];
    return undefined;
  }
        ...
        >
          <div className="flex gap-4">
            ...
            <div className="flex-shrink-0 w-16 h-24 md:w-20 md:h-28 bg-gradient-to-br from-ilaw-gold to-brand-amber rounded-lg flex items-center justify-center text-ilaw-navy shadow-lg group-hover:scale-105 transition-transform duration-300">
              {coverUrlForProgress(progress) ? (
                <img
                  src={coverUrlForProgress(progress)}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex justify-end">
                <Link
                    href={
                      // If this is one of the exclusive animated stories, link to the animated reader route
                      (() => {
                        const slug = slugFromProgress(progress);
                        if (slug && EXCLUSIVE_2D_SLUGS.has(slug)) return `/student/read-twodanimation/${slug}`;
                        return progress.book.type === "educational"
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 mb-8">
    ...
    <Card className="hover:shadow-ilaw transition-all duration-300 border-4 sm:border-6 md:border-8 border-brand-gold-400 hover:border-ilaw-gold group hover:scale-105">
     <CardContent className="p-3 sm:p-4 md:p-8 flex flex-col items-center text-center h-full bg-gradient-to-br from-ilaw-white to-brand-navy-50">
       <div className="mb-3 sm:mb-4 md:mb-6 w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 group-hover:scale-110 transition-transform duration-300">
         <img src={storybookImage} alt="Storybooks" className="w-full h-full object-contain" />
       </div>
    ...
    <Card className="hover:shadow-navy transition-all duration-300 border-4 sm:border-6 md:border-8 border-purple-400 hover:border-purple-600 group hover:scale-105">
     <CardContent className="p-3 sm:p-4 md:p-8 flex flex-col items-center text-center bg-gradient-to-br from-purple-50 to-indigo-50">
       <div className="mb-3 sm:mb-4 md:mb-6 w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 relative group-hover:scale-110 transition-transform duration-300">
