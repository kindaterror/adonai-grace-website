...
import { ChevronLeft, ChevronRight, Home, Volume2, VolumeX, Mic, MicOff, Check, RefreshCw, Maximize, Minimize, Sparkles, Mountain, Languages } from 'lucide-react';
...
type Page = { id: string; title?: string; narration: string; character?: string; illustration?: string; videoMp4?: string; videoWebm?: string; videoPoster?: string; quiz?: Quiz; audioNarration?: string };
...
function normalizeRawPage(raw: any): Page {
  ...
  return { id: raw.id, title: raw.title, narration: raw.narration || '', character: raw.character, illustration: withBase(raw.illustration), videoMp4: withBase(raw.videoMp4), videoWebm: withBase(raw.videoWebm), videoPoster: withBase(raw.videoPoster), audioNarration: raw.audioNarration };
}
...
export default function BernardoCarpioStory(){
  ...
  // Invisible reading session: start when the component mounts, and end when it unmounts or page hides.
  useEffect(() => {
    const start = async () => {
      try {
        await fetch('/api/reading-sessions/start', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('token')}` },
          body: JSON.stringify({ slug: 'bernardo-carpio' }),
        });
      } catch (e) {}
    };
    start();
    ...
    const endSession = async () => {
      try {
        const data = JSON.stringify({ slug: 'bernardo-carpio' });
        if (navigator.sendBeacon) navigator.sendBeacon('/api/reading-sessions/end', data);
        else await fetch('/api/reading-sessions/end', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('token')}` }, body: data });
      } catch (e) {}
    };

    const onVisibility = () => { if (document.visibilityState === 'hidden') endSession(); };
    window.addEventListener('visibilitychange', onVisibility);
    window.addEventListener('beforeunload', endSession as any);

    return () => { window.removeEventListener('visibilitychange', onVisibility); window.removeEventListener('beforeunload', endSession as any); endSession(); };
  }, []);
...
// environment state
    ...
    // narration state
    const [narratedPages, setNarratedPages] = useState<Set<string>>(new Set());
    const narrationAudioRef = useRef<HTMLAudioElement | null>(null);

    const playNarration = useCallback((pageId: string, audioSrc: string) => {
      if (isNarrationMuted || narratedPages.has(pageId)) return;
      ...
      audio.volume = 0.8;
      audio.play().catch(() => {});
      setNarratedPages(prev => {
        const newSet = new Set(prev);
        newSet.add(pageId);
        return newSet;
      });
    }, [isNarrationMuted, narratedPages]);

// typing cache
   const typedCacheRef = useRef<Set<string>>(new Set());

   // narration cleanup on unmount
   useEffect(() => {
     return () => {
       if (narrationAudioRef.current) {
         narrationAudioRef.current.pause();
...
/* ===== typing text helper ===== */
   const TypingText = memo(function TypingTextMemo({ text, className, speed=45, startDelay=120, cacheKey, onDone }: { text:string; className?:string; speed?:number; startDelay?:number; cacheKey:string; onDone?: () => void; }){ const already=typedCacheRef.current.has(cacheKey); const [out,setOut]=useState(already?text:''); const [done,setDone]=useState(already); useEffect(()=>{ if(already) return; setOut(''); setDone(false); const st=setTimeout(()=>{ let i=0; const id=setInterval(()=>{ i++; setOut(text.slice(0,i)); if(i>=text.length){ clearInterval(id); setDone(true); typedCacheRef.current.add(cacheKey); onDone?.(); } },speed); },startDelay); return ()=>{ clearTimeout(st); }; },[text,speed,startDelay,cacheKey,already,onDone]); return <span className={`nc-typing ${className||''}`}>{out}{!done && <span className="nc-caret" aria-hidden>|</span>}</span>; });

/* ===== page block ===== */
   const PageBlock = ({ page }: { page: Page }) => {
     const handleNarrationDone = () => {
       if (page.audioNarration) {
         playNarration(page.id, page.audioNarration);
       }
     };
     const renderTyped = (fieldKey:string, text?:string) => { if(!text) return null; const key = `${page.id}-${fieldKey}`; if(typedCacheRef.current.has(key)) return <span>{text}</span>; return <TypingText text={text} cacheKey={key} onDone={handleNarrationDone} />; };
     return (
...
{page.audioNarration && (
              <button
                ...
                className="nc-narration-btn absolute top-2 right-2 p-1.5 rounded-full bg-white/20 hover:bg-white/40 transition"
                aria-label="Play narration"
              >
                <Volume2 size={16} className="text-indigo-700" />
              </button>
            )}
         </div>
  ...
  const restartStory = () => { setSpreadStart(0); setRightUnlocked(false); setPendingSide(null); setQuizPageIndex(null); setIsStoryComplete(false); setFlipRightNow(false); setAnswers({}); setNarratedPages(new Set()); setIsNarrationMuted(false); resetQuiz(); resetCheckpointAPI(BOOK_SLUG).catch(()=>{}); saveCheckpoint().catch(()=>{}); };
          ...
          <div className='w-full max-w-7xl'>
            <div className='flex justify-between items-center mb-6'>
              ...
              <div className='flex gap-2'>
                ...
                <Button onClick={()=> setIsMuted(v=>!v)} className='kid-btn btn-solid-white' aria-label={isMuted ? 'Unmute background music' : 'Mute background music'}>{isMuted? <VolumeX size={16}/> : <Volume2 size={16}/>}</Button>
                <Button onClick={()=> setIsNarrationMuted(v=>!v)} className='kid-btn btn-solid-white' aria-label={isNarrationMuted ? 'Unmute narration' : 'Mute narration'}>{isNarrationMuted? <MicOff size={16}/> : <Mic size={16}/>}</Button>
              </div>
            </div>
          </div>
        )}
