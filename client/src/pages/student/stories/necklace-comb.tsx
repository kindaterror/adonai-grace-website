...
import { ChevronLeft, ChevronRight, Home, Volume2, VolumeX, Mic, MicOff, Check, Maximize, Minimize, RefreshCw, Sparkles, Moon, Gem, Languages } from 'lucide-react';
...
type Page = { id: string; narration?: string; character?: string; illustration?: string; videoMp4?: string; videoWebm?: string; videoPoster?: string; videoInitialStart?: number; videoInitialEnd?: number; videoLoopStart?: number; videoLoopEnd?: number; videoPlaybackRate?: number; live2dModelUrl?: string; live2dEntryMotion?: string; live2dIdleMotion?: string; live2dScale?: number; live2dFitPadding?: number; live2dFitMode?: string; live2dOffsetX?: number; live2dOffsetY?: number; live2dBgModelUrl?: string; live2dBgEntryMotion?: string; live2dBgIdleMotion?: string; live2dBgScale?: number; live2dBgOffsetX?: number; live2dBgOffsetY?: number; quiz?: Quiz; audioNarration?: string; };
...
export default function NecklaceCombStory() {
  ...
  // Start invisible reading session for this slug on mount; end on unmount/visibility
  useEffect(() => {
    const start = async () => {
      try {
        await fetch('/api/reading-sessions/start', {
          ...
          body: JSON.stringify({ slug: 'necklace-comb' }),
        });
      } catch (e) {}
    };
    start();
    ...
    const endSession = async () => {
      try {
        const data = JSON.stringify({ slug: 'necklace-comb' });
    ...
    return () => {
      ...
      window.removeEventListener('beforeunload', endSession as any);
      endSession();
    };
  }, []);
...
const restartStory = () => {
     ...
     setAnswers({});
     resetQuiz();
     setNarratedPages(new Set()); // Reset narration state
     setIsNarrationMuted(false); // Reset narration mute
...
{/* narration button */}
          {page.audioNarration && (
            <button
              ...
              aria-label={lang === 'en' ? 'Play narration' : 'I-play ang narration'}
              title={lang === 'en' ? 'Play narration' : 'I-play ang narration'}
            >
        ...
        : i18n[lang].pageOf(
            ...
            String(storyPages.length)
                ...
                <div className="flex gap-2">
                  ...
                  <Button onClick={() => setIsMuted(v=>!v)} className="kid-btn btn-solid-white" aria-label={isMuted ? 'Unmute background music' : 'Mute background music'}>
                    {isMuted ? <VolumeX size={16} /> : <Volume2 size={16} />}
                  </Button>
                  <Button onClick={() => setIsNarrationMuted(v=>!v)} className="kid-btn btn-solid-white" aria-label={isNarrationMuted ? 'Unmute narration' : 'Mute narration'}>
