...
type Page = { id: string; title?: string; narration: string; character?: string; illustration?: string; quiz?: Quiz; videoMp4?: string; videoWebm?: string; videoPoster?: string; videoInitialStart?: number; videoInitialEnd?: number; videoLoopStart?: number; videoLoopEnd?: number; videoPlaybackRate?: number; audioNarration?: string };
...
export default function SunMoonStory() {
  ...
  // Invisible reading session (no UI) — start on mount, end on unmount/visibility
  useEffect(() => {
    let active = true;
    const start = async () => {
      try {
        await fetch('/api/reading-sessions/start', {
          ...
          body: JSON.stringify({ slug: BOOK_SLUG }),
        });
      } catch (e) {}
    };
    start();
    ...
    const endSession = async () => {
      try {
        const data = JSON.stringify({ slug: BOOK_SLUG });
        // prefer sendBeacon for unload
        if (navigator.sendBeacon) {
          navigator.sendBeacon('/api/reading-sessions/end', data);
        } else {
          await fetch('/api/reading-sessions/end', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('token')}` }, body: data });
        }
      } catch (e) {}
    };
...
const [feedback, setFeedback] = useState("");
  ...
  // Narration audio cleanup on unmount
  useEffect(() => {
    return () => {
      if (narrationAudioRef.current) {
        narrationAudioRef.current.pause();
...
const PageBlock = memo(function PageBlock({ page, index, isCritical }: { page: Page; index: number; isCritical?: boolean }) {
     ...
     const renderTyped = (fieldKey: string, text: string, className: string) => {
       ...
       return <TypingText text={text} className={className} cacheKey={key} onDone={handleNarrationDone} />;
     };
            ...
            <div className="w-full max-w-7xl">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                  ...
                  <Button onClick={() => setIsMuted((v) => !v)} className="sm-btn btn-white" aria-label={isMuted ? 'Unmute background music' : 'Mute background music'}>
                    {isMuted ? <VolumeX size={16} /> : <Volume2 size={16} />}
                  </Button>
                  <Button onClick={() => setIsNarrationMuted((v) => !v)} className="sm-btn btn-white" aria-label={isNarrationMuted ? 'Unmute narration' : 'Mute narration'}>
