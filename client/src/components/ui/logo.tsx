...
export function Logo({ 
  ...
  const getVariantStyles = () => {
    switch (variant) {
      ...
      case "admin":
        // Updated to match your new slate/blue Admin theme
        return "text-slate-900 hover:text-blue-600 transition-colors duration-300";
      case "teacher":
    ...
    <div className={`flex items-center group cursor-pointer ${className}`} {...props}>
      ...
      {showText && (
        <div className="ml-3 flex flex-col">
          <span className={`font-sans font-bold leading-tight ${getTextSizeClasses()} ${getVariantStyles()}`}>
            {SCHOOL_NAME_SHORT}
          </span>
          ...
          {(size === "lg" || size === "xl") && (
            <span className={`font-sans font-medium italic ${
