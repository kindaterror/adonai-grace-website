...
function isHttpUrl(url?: string | null) {
  if (!url) return false;
  // Accept absolute http(s), root-relative (/path/to/img), relative (./, ../), data:, and blob: URLs.
  // This prevents rejected local static assets like '/book-image/...' while still guarding against null/empty strings.
