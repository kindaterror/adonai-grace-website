...
export function AccountActions({ userRole, user }: AccountActionsProps) {
  ...
  // == Role-based styling (admin updated to slate/clean theme) ==
  const getRoleStyles = () => {
    switch (userRole) {
      case "admin":
        return {
          ...
          primaryBtn:
            "bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 shadow-sm font-medium",
          subtleCard: "bg-white border border-slate-200 shadow-sm",
          subtleCardTitle: "text-slate-900",
          subtleCardText: "text-slate-500",
          navyCard: "bg-slate-50 border border-slate-200 shadow-sm",
          navyBtn:
            "bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 shadow-sm font-medium",
        };
      case "teacher":
        return {
  ...
  return (
    ...
    >
      <div className="flex items-center mb-6">
        ...
        <h2 className={`text-xl font-bold ${s.headerTitle}`}>Account Actions</h2>
      </div>

      <div className="space-y-6">
        {/* Export Data Section */}
        <motion.div
          className={`rounded-xl p-5 ${s.subtleCard}`}
          variants={fadeInUp}
          initial="hidden"
          animate="visible"
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              <h3 className={`font-semibold mb-2 ${s.subtleCardTitle}`}>
                Export Your Data
              </h3>
        ...
        {/* Logout All Devices */}
        <motion.div
          className={`rounded-xl p-5 ${s.navyCard}`}
          variants={fadeInUp}
          initial="hidden"
          animate="visible"
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              <h3 className={`font-semibold mb-2 ${s.subtleCardTitle || "text-ilaw-navy"}`}>Security Action</h3>
              <p className={`text-sm mb-4 ${s.subtleCardText || "text-ilaw-navy/80"}`}>
              ...
              <AnimatePresence initial={false}>
                {!showDeleteConfirm ? (
                  <motion.button
                    ...
                    className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg inline-flex items-center font-medium shadow-sm"
                  >
                  ...
                  >
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 border border-red-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 shadow-sm"
                    ...
                    <div className="flex flex-wrap gap-3">
                      <motion.button
                        ...
                        className={`px-4 py-2 rounded-lg inline-flex items-center font-medium shadow-sm ${
                      ...
                      <motion.button
                        ...
                        className="px-4 py-2 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium rounded-lg shadow-sm"
                      >
                        Cancel
                      </motion.button>
                    </div>
